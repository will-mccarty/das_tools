import ioda_access as iod
import numpy as np

def omf(data=None,return_deps=False,obs_var=None):
    if obs_var is not None:
        bkgvar = iod.var_to_var('hofx/OBSVAR',obs_var=obs_var)#.replace('OBSCAR',obs_var)
        obsvar = iod.var_to_var('ObsValue/OBSVAR',obs_var=obs_var)#.replace('OBSCAR',obs_var)
    else:
        print('warning, obs_var not set!')

    val = data[obsvar] - data[bkgvar]
    return(val)

def gsiomfbc(data=None,return_deps=False,obs_var=None):
    if obs_var is not None:
        bkgvar = iod.var_to_var('GsiHofXBc/OBSVAR',obs_var=obs_var)#.replace('OBSCAR',obs_var)
        obsvar = iod.var_to_var('ObsValue/OBSVAR',obs_var=obs_var)#.replace('OBSCAR',obs_var)
    else:
        print('warning, obs_var not set!')
...
def gsiomfnbc(data=None,return_deps=False,obs_var=None):
    if obs_var is not None:
        bkgvar = iod.var_to_var('GsiHofX/OBSVAR',obs_var=obs_var)#.replace('OBSCAR',obs_var)
        obsvar = iod.var_to_var('ObsValue/OBSVAR',obs_var=obs_var)#.replace('OBSCAR',obs_var)
    else:
        print('warning, obs_var not set!')
...
#    deps = ['omf','oma']
#
#    val = data[iod.var_to_var('omf')]-data[iod.var_to_var('oma')]
#    return(val)
#
#def omfbyo(data=None,return_deps=False):
#    deps = ['omf','obs']
#
#    omf = data[iod.var_to_var('omf')]
#    obs = data[iod.var_to_var('obs')]
#
#    val = omf/obs
#    return(val)
#
...
#    deps = ['omf','obs']
#   
#    val = data[iod.var_to_var('omf')] / (data[iod.var_to_var('obs')] - data[iod.var_to_var('omf')])
#    return(val)
#
#
#
...
#    deps = ['omfbc','omfnbc']
#
#    val = data[iod.var_to_var('omfbc')]-data[iod.var_to_var('omfnbc')]
#    return(val)
#
#def fcst(data=None,return_deps=False):
#    deps = ['omf','obs']
#
#    val = data[iod.var_to_var('obs')]-data[iod.var_to_var('omf')]
#    return(val)
#
#def anl(data=None,return_deps=False):
#    deps = ['oma','obs']
#
#    val = data[iod.var_to_var('obs')]-data[iod.var_to_var('oma')]
#    return(val)
#
#
#def spd_omf(data=None,return_deps=False):
#    deps = ['u_obs','v_obs','u_omf','v_omf']
#    u_bkg = data[iod.var_to_var('u_obs')] - data[iod.var_to_var('u_omf')] 
#    v_bkg = data[iod.var_to_var('v_obs')] - data[iod.var_to_var('v_omf')]
#    spd_o = (data[iod.var_to_var('u_obs')]**2 + data[iod.var_to_var('v_obs')]**2) ** (0.5)
...
#    deps = ['u_obs','v_obs']
#    u_obs = data[iod.var_to_var('u_obs')]
#    v_obs = data[iod.var_to_var('v_obs')]
#
...
#       raise Exception('Distance calculations require a focal lon/lat to be set by iod.set_centroid(lon,lat)')
#    d = ( (data[iod.var_to_var('lat')] - data['centroid_lat']) ** 2 + (data[iod.var_to_var('lon')] - data['centroid_lon']) ** 2 ) ** (0.5) 
##    d = cdist(np.transpose( [data[iod.var_to_var('lat'), data[iod.var_to_var('lon')] ), [data['centroid_lat', data['centroid_lon'], 'euclidean')
#    return(d)
#
...
#    deps = ['lon']
#
#    nv = data[iod.var_to_var('lon')].size
#    val = rnd.rand(nv)
#    return(val)
#
